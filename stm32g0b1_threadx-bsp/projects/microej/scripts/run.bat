...
"%IAREW_INSTALLATION_DIR%\common\bin\cspybat" --download_only "%IAREW_INSTALLATION_DIR%\arm\bin\armproc.dll" "%IAREW_INSTALLATION_DIR%\arm\bin\armstlink2.dll" "%IAREW_PROJECT_DIR%\%IAREW_PROJECT_CONFIGURATION%\EXE\%IAREW_PROJECT_NAME%.out"  --plugin="%IAREW_INSTALLATION_DIR%\arm\bin\armbat.dll" --device_macro="%IAREW_INSTALLATION_DIR%\%IAREW_PROJECT_DEVICE_MACRO%" --flash_loader="%IAREW_INSTALLATION_DIR%\%IAREW_PROJECT_FLASH_LOADER%" --backend "--endian=little" "--cpu=Cortex-M0+"  "--fpu=None" "-p"  "%IAREW_INSTALLATION_DIR%\arm\CONFIG\debugger\ST\STM32G0B1RE.ddf" "--semihosting" "--device=STM32G0B1RE" "--drv_interface=SWD" "--stlink_reset_strategy=0,0" "--drv_swo_clock_setup=64000000,0,2000000" "--drv_catch_exceptions=0x000" "--drv_debug_ap=0" 